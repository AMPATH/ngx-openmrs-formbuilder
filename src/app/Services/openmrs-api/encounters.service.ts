...
export class EncounterService {
    ...
    getEncounterByUuid(encounterUuid: string) {
        const v = 'custom:(uuid,encounterDatetime,' +
        ...
        return this.http.get<any>(`${this.restUrl}/${encounterUuid}?v=${v}`, {headers: this.headers});
    }
    ...
    public getEncountersByPatientUuid(patientUuid: string): Observable<any> {
        ...
        return this.http.get<any>(this.restUrl + '?patient=' + patientUuid, {headers: this.headers}).pipe(
    ...
    saveEncounter(payload: any) {
        ...
        return this.http.post<any>(this.restUrl, JSON.stringify(payload), {headers: this.headers});
    }
}
