...
export class SaveFormService {
 ...
 constructor(private http:Http, private sessionStorageService:SessionStorageService){
  ...
  uploadSchema(schema){
    ...
    return this.http.post(`${this.baseUrl}/ws/rest/v1/clobdata`,body,{headers:this.headers}).pipe(map(res => JSON.parse(JSON.stringify(res))));
  }
  ...
  getResourceUUID(formUUID,valueReference){
    ...
    return this.http.post(`${this.baseUrl}/ws/rest/v1/form/${formUUID}/resource`,body,{headers:this.headers}).pipe(map(res => JSON.parse(JSON.stringify(res))));
  }
  ...
  saveNewForm(name,version,published?,description?,encounterType?){
      ...
      return this.http.post(`${this.baseUrl}/ws/rest/v1/form`,body,{headers:this.headers}).pipe(map(res => JSON.parse(JSON.stringify(res))));
    }
    ...
    publish(uuid){
        ...
        return this.http.post(`${this.baseUrl}/ws/rest/v1/form/${uuid}`,body,{headers:this.headers}).pipe(map(res => res.json()));
    }
    ...
    updateEncounterType(encounterType: any, formUuid: string){
        ...
        return this.http.post(`${this.baseUrl}/ws/rest/v1/form/${formUuid}`, body, {headers: this.headers}).pipe(map(res => res.json()));
    }
