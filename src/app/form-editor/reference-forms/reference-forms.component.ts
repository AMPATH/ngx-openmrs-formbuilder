...
export class ReferenceFormsComponent implements OnInit {
  ...
  constructor(
    private fas: FetchAllFormsService,
  ...
  ngOnInit() {
    this.fas
      ...
      .subscribe((res) => (this.componentForms = res.results));

    this.ns
      .getRawSchema()
      .subscribe((res) => (this._rawSchema = _.cloneDeep(res)));
  ...
  insertRefForm() {
    this.ds
      .addDialog(
        ...
        { forms: this.componentForms },
        { backdropColor: 'rgba(0, 0, 0, 0.5)' }
      )
      .subscribe((selectedForms) => {
        if (selectedForms) {
          this.refForm.formName = selectedForms['form'].substring(
            selectedForms['form'].indexOf(' ') + 1
          );
          this.refForm.alias = selectedForms['alias'];
          this.refForm.ref['display'] = selectedForms['display'];
          this.refForm.ref['uuid'] = selectedForms['form'].substring(
            0,
            selectedForms['form'].indexOf(' ')
          );
