...
export class UpdateFormsWizardComponent implements OnInit {
  ...
  head:Node;
  currentNode:Node;
  @Input() set _newComponentMetadata(data) { this.componentMetadata = data;  } // for the new component form
  @Input() set _selectedForms(data) { this.schemaMetadatas = data; }
  @Input() set _oldComponentMetadata(data) { this.oldComponentUUID = data;}
  @Output() finished:EventEmitter<boolean> = new EventEmitter();
  constructor(private updateComponentService:UpdateComponentService,private saveFormService:SaveFormService,private snackbar:MdSnackBar,
     private fetchFormDetailService:FetchFormDetailService,private ns:NavigatorService, compiler:FormSchemaCompiler,private formListService:FormListService) { }
  ...
  replaceOldComponent(schema:any,oldComponentUUID:string,newComponentMetadata:any){
    ...
    this.fetchFormDetailService.fetchReferencedFormSchemas(schema.referencedForms).then((schemas) => {
  ...
  saveForm(schema:any,metadata:any,newVersion:number){
          let strNewVersion = newVersion.toString();
          let form = metadata;
  ...
  showDoneSnackBar(name){
    this.snackbar.open("Updated "+name, '' ,{duration:1200});
  }
